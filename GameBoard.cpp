...
GameBoard::GameBoard() : grid(nullptr) {
    ...
    grid = std::make_unique<std::unique_ptr<Cell[]>[]>(GameSession::BOARD_SIZE);
    //  Для каждой строки создаем массив ячеек
    for (int i = 0; i < GameSession::BOARD_SIZE; i++) {
        grid[i] = std::make_unique<Cell[]>(GameSession::BOARD_SIZE);
...
{
    return x >= 0 && x < GameSession::BOARD_SIZE && y >= 0 && y < GameSession::BOARD_SIZE;
}
...
bool GameBoard::canPlaceShip(int x, int y, int size, bool horizontal) const
{
    ...
    if (horizontal) {
        if (y + size > GameSession::BOARD_SIZE) {
            return false; // Корабль выходит за правую границу
        }
    }
    else {
        if (x + size > GameSession::BOARD_SIZE) {
...
bool GameBoard::isAreaClear(int x, int y, int size, bool horizontal) const
{
    ...
    int endX = min(GameSession::BOARD_SIZE - 1, horizontal ? x + 1 : x + size);
    int startY = max(0, y - 1);
    int endY = min(GameSession::BOARD_SIZE - 1, horizontal ? y + size : y + 1);
